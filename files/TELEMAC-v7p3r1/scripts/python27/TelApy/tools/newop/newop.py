...
class Newop(object):
    ...
    """
    def __init__(self, dx=1.e-6, maxfun=2000, verbose=True):
        """
    ...
    def optimize(self, varx0, niter=100, nproc=1):
        ...
        res = minimize(numv, varx0, args=(nproc, self.dx, self.vdx, \
                self.function), \
                bounds=self.bounds, method='L-BFGS-B', \
                jac=True, \
                options={'maxiter':niter, 'maxfun':self.maxfun, \
                'disp':self.verbose})
