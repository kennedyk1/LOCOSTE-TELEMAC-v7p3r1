...
def create_tmp_folder(tmp_path,data_path):
    ...
    _, subdirs, files = walk(data_path).next()
    for fle in files:
...
def dump_output_data({outputs}, res_file):
...
def study_function({inputs}):
    ...
    user_fortran = None if '{user_fortran}' == 'None' else '{user_fortran}'.replace(case_dir+sep,'')
    ...
    # Instance delete
    del(my_case)
    dump_output_data({outputs}, res_file)
    # Creating a temporary folder to run the study
    tmp_dir = delete_tmp_folder(tmp_dir, '{res_path}', random_string, res_file)
...
def generate_set(input_variable):
    ...
    if 'INDEX' in input_variable['VAR_INFO']['ZONE_DEF']:
        ...
        string = "my_case.set('%s', %s, i=%i, j=%i, k=%i)"%(var_name, name, i, j, k)
    elif 'RANGE' in input_variable['VAR_INFO']['ZONE_DEF']:
        irange = input_variable['VAR_INFO']['ZONE_DEF']['RANGE']
        string = "my_case.set_on_range('%s', %s, '%s')"%(var_name, name, irange)
    elif 'POLYGON_FILE' in input_variable['VAR_INFO']['ZONE_DEF']:
        poly_file = input_variable['VAR_INFO']['ZONE_DEF']['POLYGON_FILE']['FILE_NAME']
        poly_sep = input_variable['VAR_INFO']['ZONE_DEF']['POLYGON_FILE']['SEPARATOR']
        poly = import_poly_from_file(poly_file, poly_sep)
        string = "my_case.set_on_polygon('%s', %s, %s)"%(var_name, name, str(poly))
...
def generate_get(output_variable):
    ...
    if 'INDEX' in output_variable['VAR_INFO']['ZONE_DEF']:
        ...
        string = "%s = my_case.get('%s', i=%i, j=%i, k=%i)"%(name, var_name, i, j, k)
    else:
...
def handle_variables(jdc):
    ...
    for var in input_vars:
        ...
        if pos == NO_POSITION:
           raise Exception(["Unknown position for "+var])
...
def get_result_keyword(module):
    ...
    if module == 'Telemac2d':
        ...
        raise Exception(["module "+module+" is not handled by generate_study yet!"])
...
def generate_study_script(jdc):
    ...
    inputs = ["%s=%s"%input_data for input_data in zip(input_names, input_val)]

    # Adding result file if in the catalog
    res_folder = jdc.get('RESULT_FOLDER', path.dirname(steering_file))

    if 'RESULTS_FILE_NAME' in jdc:
        getset[RUN_READ_CASE_POS] += set_result_file(module, jdc['RESULTS_FILE_NAME']) + linesep + ' '*4
...
def generate_study_yacs(jdc):
    ...
    try:
        ...
        raise Exception(["Missing Salome this function is only available within Salome"])
