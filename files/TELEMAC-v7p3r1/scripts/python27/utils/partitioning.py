...
class splitSELAFIN():
   ...
   def __init__(self,SLFfileName,CLMfileName,SEQfileName='',splitCONLIM=False,DOMfileRoot=''):
      ...
      print( '\n... Split by elements in '+str(self.NPARTS)+' parts\n' )
               ...
               for p1,p2,p3 in zip([0,1,2],[1,2,0],[2,0,1]):
                  if NSPLIT[e[p1]] != KSPLIT[k] and NSPLIT[e[p2]] != KSPLIT[k]:
                     if IPOBO[e[p1]] != 0 and IPOBO[e[p2]] != 0:
                        print( '       ~> correcting boundary segment at iteration: '+str(nloop)+' '+repr((e[p1],e[p2]))+' '+str(k)+' '+repr(KSPLIT[k])+' '+str(e)+' '+repr(NSPLIT[e]) )
      ...
      while found:
         ...
         for k in range(len(self.slf.IKLE)):
            ...
            if min( NSPLIT[e] ) != max( NSPLIT[e] ) and KSPLIT[k] != min( NSPLIT[e] ):
               print( '       ~> correcting internal segment at iteration: '+str(nloop)+' '+str(k)+' '+repr(KSPLIT[k])+' '+str(e)+' '+repr(NSPLIT[e]) )
            ...
            while w < len(HLINK):
               if HLINK[w] < len(HLINK):
                  if INTER['h'][HEADT[w]] == INTER['t'][HEADT[HLINK[w]]] and INTER['t'][HEADT[w]] == INTER['h'][HEADT[HLINK[w]]]:
                     print( '       ~> Removing dupicate segments in part: '+str(part)+' '+repr(SINTER[part][HEADT[w]])+' '+repr(SINTER[part][HEADT[HLINK[w]]]) )
      ...
      for k in KIKLE:
         ...
         for p1,p2 in zip([0,1,2],[1,2,0]):
            if MASKER[e[p1]] > 0 and MASKER[e[p2]] > 0 and MASKER[e[p2]] > MASKER[e[p1]]:
               print( '       ~> Warning for element of part: '+str(part)+' (was:'+repr(KSPLIT[k])+') '+str(k)+' '+str(e) )
               #KSPLIT[k] = part
         pbar.update(k)
      pbar.finish()
   ...
   def putContent(self):
      ...
      for part in range(self.NPARTS): # this could be done in parallel
         ...
         print( '    +> Joining up boundary segments for part: '+str(part+1) )
      ...
      if self.isDOMAIN != '':
         ...
         for part in range(self.NPARTS):
            print( '    +> part '+str(part+1)+' of '+str(self.NPARTS) )
            polyXY = []
      ...
      for part in range(self.NPARTS):
         print( '    +> re-ordering IKLE for part '+str(part+1) )
      ...
      for i in ISEG:
         if len(ISEG[i]) != 2:
            print( '... You have a boundary node surounded with more than two boundary segments: '+str(i) )
            sys.exit(1)
         parts = ISEG[i].keys()
