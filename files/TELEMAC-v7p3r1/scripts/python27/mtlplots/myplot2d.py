            ...
            if type(mpl.rcParams[key]) == type([]):
               if type(mpl.rcParams[key][0]) == type(1) or type(mpl.rcParams[key][0]) == type(1.0):
                  ...
                  print('... I did not know ' + type(
                     mpl.rcParams[key]) + ' for key: ' + key + '. Could be an acceptable type')
                  sys.exit(1)
...
def drawColouredTriMaps(myplt, decoUser, (x, y, ikle, z)):
   ...
   # add variables and option for colorbar
   colorBar = False
   title = None
   if 'colorbar' in decoUser.keys() and decoUser['colorbar']:
      colorBar = True
      decoColorbar = {}
      decoUser.pop('colorbar', None)
      paramColorbar = ['orientation', 'fraction', 'pad', 'shrink', 'aspect', 'anchor', 'panchor', 'format']
      for key in decoUser.keys():
         if key in paramColorbar:
            decoColorbar[key] = decoUser[key]
            decoUser.pop(key, None)
         elif key == 'title_colorbar':
            title = decoUser[key]
            decoUser.pop(key, None)
   ...
   cs = myplt.tricontourf(triang, z, **decoUser)

   cbar = None
   if colorBar:
      cbar = myplt.colorbar(**decoColorbar)
...
def drawColouredTriVects(myplt, decoUser, (x, y, uv, normalised, ikle)):
   """
   ...
   decoVector = {}
   decoKey = {}
   val_Key = {'X': 0.8, 'Y':0.05, 'U':1.0, 'label':''}
   grid_deltaX = None
   grid_deltaY = None
   ...
   for key in decoUser:
      if (key in vector_dict):
         if key == ('scale' or 'width' or
                     ...
                     'linewidths'):
            decoVector[key] = float(decoUser[key])
         elif key == 'grid_delta':
         ...
         if key == 'key_x':
            val_Key['X'] = float(decoUser[key])
         elif key == 'key_y':
            val_Key['Y'] = float(decoUser[key])
         elif key == 'key_length':
            val_Key['U'] = float(decoUser[key])
         elif key == 'key_label':
            val_Key['label'] = decoUser[key]
         elif key == 'key_color':
            decoKey['color'] = decoUser[key]
         else:
            try:
               decoKey[key] = eval(decoUser[key])
            except:
               decoKey[key] = decoUser[key]
   ...
   if not normalised:
      ...
      cs = myplt.quiver(x, y, uv[0]/norm, uv[1]/norm, **decoVector)

   # ARI - ADLANE REBAI - ARTELIAGROUP
   # USing set_array will color the arrows
   # cs.set_array(z)
   ...
   if 'key' in decoUser and decoUser['key'] == "yes":
      myplt.quiverkey(cs, val_Key['X'], val_Key['Y'], val_Key['U'], val_Key['label'],
                  **decoKey)
...
class Figure2D(Caster):
   def __init__(self, caster, plot):
      ...
      fig.add_subplot(111)  # deprecated : ,axisbg=background)
      ax1, = fig.get_axes()
      ...
      # ~~> user params
      decotitle = {}
      if 'title_fontsize' in self.upar and self.upar['title_fontsize'] is not None:
         decotitle['fontsize'] = self.upar['title_fontsize']
         self.upar.pop('title_fontsize', None)
      if self.upar['title'] != '':
         plt.title(self.upar['title'], **decotitle)
      # ~~> type of plot
      if self.upar['type'] != '':
         ...
         ax1.set_aspect(self.upar['aspect'])
      if 'xticks_fontsize' in self.upar and self.upar['xticks_fontsize'] is not None:
         for tick in ax1.xaxis.get_major_ticks():
            tick.label.set_fontsize(self.upar['xticks_fontsize'])
         for tick in ax1.yaxis.get_major_ticks():
            tick.label.set_fontsize(self.upar['xticks_fontsize'])
         self.upar.pop('xticks_fontsize', None)
   ...
   def add(self, typl, what):
      ...
      if 'tif' in typl.lower():
         ...
         # ~~> Extract variable data for only one time frame and one plane
            ...
            # ~~> Draw/Dump (multiple options possible)
            if "map" in vtype:
               self.cbar = drawColouredTriMaps(self.plt, what['deco'], (MESHX, MESHY, IKLE3, VARSORS[0]))
            elif "label" in vtype:
               drawLabeledTriContours(self.plt, what['deco'], (MESHX, MESHY, IKLE3, VARSORS[0]))
            elif "arrow" in vtype or "vector" in vtype:
               drawColouredTriVects(self.plt, what['deco'], (MESHX, MESHY, VARSORS, False, IKLE3))
            elif "angle" in vtype:
               drawColouredTriVects(self.plt, what['deco'], (MESHX, MESHY, VARSORS, True, IKLE3))
            else:
