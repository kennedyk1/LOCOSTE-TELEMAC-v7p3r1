         ...
         if type(mpl.rcParams[key]) == type(upar[key]):
            ...
            if type(mpl.rcParams[key]) == type([]):
               if type(mpl.rcParams[key][0]) == type(1) or type(mpl.rcParams[key][0]) == type(1.0):
                  ...
                  err = '... I did not know {} for key: {}. Could be an acceptable type'.format(str(type(mpl.rcParams[key])))
                  print(err)
                  sys.exit(1)
...
def drawHistoryLines(myplt, decoUser, x, ys):
   ...
   if dim == 1:
      ...
      for i0 in range(len(n0)):  # each variables
         myplt.ylabel(n0[i0])
         l = myplt.plot(x0, y0[i0], **decoUser)
         if type(l) is list:
      ...
      for i1 in range(len(n1)):  # each location
         for i0 in range(len(n0)):  # each variables
            ...
            l = myplt.plot(x0, y0[i0][i1], **decoUser)
            if type(l) is list:
               line_tempo.append(l[0])
            else:
               line_tempo.append(l)
      ...
      for i2 in range(len(n2)):  # each plan
         for i1 in range(len(n1)):  # each time
            for i0 in range(len(n0)):  # each variables
               # myplt.ylabel(str(n1[])) you could label each curve in time and plan
               l = myplt.plot(x0, y0[i0][i1][i2], **decoUser)
               if type(l) is list:
                  line_tempo.append(l[0])
               else:
                  line_tempo.append(l)
         ...
         for i2 in range(len(n2)):
            for i1 in range(len(n1)):
               for i0 in range(len(n0)):
                  # myplt.ylabel(str(n1[]))
                  l = myplt.plot(x0, y0[i0][i1][i2][i3], **decoUser)
                  if type(l) is list:
                     line_tempo.append(l[0])
                  else:
                     line_tempo.append(l)
      ...
      for i0 in range(len(n0)):  # each variables
         ...
         if 'fill_between' in decoUser and decoUser['fill_between'] is not None:
            ...
            if fill_min is not None:
               myplt.fill_between(x0, y0[i0], fill_min, **decoUser)
            else:
               myplt.fill_between(x0, y0[i0], min(y0[i0]), **decoUser)
         else:
      ...
      for i1 in range(len(n1)):  # each time
         for i0 in range(len(n0)):  # each variables
            ...
            if 'fill_between' in decoUser and decoUser['fill_between'] is not None and decoUser['fill_between']:
               decoUser.pop('fill_between', None)
               if fill_min is not None:
                  l = myplt.fill_between(x0, y0[i0][i1], fill_min, **decoUser)
               else:
                  l = myplt.fill_between(x0, y0[i0][i1], min(y0[i0][i1]), **decoUser)
            else:
               l = myplt.plot(x0, y0[i0][i1], **decoUser)
            if type(l) is list:
               line_tempo.append(l[0])
         ...
         for i1 in range(len(n1)):  # each time
            for i0 in range(len(n0)):  # each variables
               ...
               if 'fill_between' in decoUser and decoUser['fill_between'] is not None and decoUser['fill_between']:
                  decoUser.pop('fill_between', None)
                  if fill_min is not None:
                     l = myplt.fill_between(x0, y0[i0][i1][i2], fill_min, **decoUser)
                  else:
                     l = myplt.fill_between(x0, y0[i0][i1][i2], min(y0[i0][i1][i2]), **decoUser)
               else:
                  l = myplt.plot(x0, y0[i0][i1][i2], **decoUser)
               if type(l) is list:
                  line_tempo.append(l[0])
            ...
            for i1 in range(len(n1)):
               for i0 in range(len(n0)):
                  ...
                  if 'fill_between' in decoUser and decoUser['fill_between'] is not None and decoUser['fill_between']:
                     decoUser.pop('fill_between', None)
                     if fill_min is not None:
                        l = myplt.fill_between(x0, y0[i0][i1][i2][i3], fill_min, **decoUser)
                     else:
                        l = myplt.fill_between(x0, y0[i0][i1][i2][i3], min(y0[i0][i1][i2][i3]), **decoUser)
                  else:
                     l = myplt.plot(x0, y0[i0][i1][i2][i3], **decoUser)
                  if type(l) is list:
                     line_tempo.append(l[0])
...
class Figure1D(Caster):
   def __init__(self, caster, plot):
      ...
      #
      if self.upar.get('xlabel', '') != '':
         plt.xlabel(self.upar['xlabel'])
      if self.upar.get('ylabel', '') != '':
         plt.ylabel(self.upar['ylabel'])
   ...
   def add(self, typl, what):
      ...
      if what['type'].split(':')[1] == 'history' or 'sortie' in typl.lower():
         cast = self.get(typl, what)
         # try: # if instance unit exist
         valout = drawHistoryLines(self.plt, what['deco'], (cast.unit, cast.support), (cast.function, cast.values))
      ...
      # ~~> 1D time history from 2D or 3D results
         ...
         cast = self.get(typl, what)
         try:  # if instance unit exist
            valout = drawPolylineLines(self.plt, self.ax, what['deco'],
   ...
   def save(self, fileName):
      ...
      legend = ()
      dico_extra = {}
      if 'legend' in self.upar:
         if eval(self.upar['legend']):
            if self.label is not None:
               if 'legend.borderaxespad' not in self.upar:
                  self.upar['legend.borderaxespad'] = None
               if 'legend.ncol' not in self.upar:
                  self.upar['legend.ncol'] = 1
               if 'legend.position' not in self.upar:
                  self.upar['legend.position'] = 0
               legend = self.ax.legend(self.lines,
                                 self.label,
                                 loc=self.upar['legend.position'],
                                 ncol=int(self.upar['legend.ncol']),
                                 borderaxespad=self.upar['legend.borderaxespad'])

            dico_extra['bbox_extra_artists'] = (legend,)
         dico_extra['bbox_inches'] = 'tight'
      if 'xlim' in self.upar and self.upar['xlim'] != '':
         self.plt.xlim(eval(self.upar['xlim']))
      if 'ylim' in self.upar and self.upar['ylim'] != '':
         self.plt.ylim(eval(self.upar['ylim']))
