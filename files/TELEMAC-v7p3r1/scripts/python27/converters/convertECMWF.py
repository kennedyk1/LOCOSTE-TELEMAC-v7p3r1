...
class ECMWF():
   ...
   def appendCoreVarsECMWF(self,ecmwfdata,itime):
      ...
      if self.typ == 'wave':
         # ~~> WAVE HEIGHT == 'swh'
         var2d = np.swapaxes( ecmwfdata.variables['swh'][itime][:], 0,1).ravel()
         varof = ecmwfdata.variables['swh'].add_offset
         varsf = ecmwfdata.variables['swh'].scale_factor
...
---------'''))
   ...
   parser.add_argument(\
      ...
      choices=set(("interim", "era5")),
      help="type of dataset requested either 'interim' or 'era5', depending on periods and resolution, etc.")
   parser.add_argument(\
