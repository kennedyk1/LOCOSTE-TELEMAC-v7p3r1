      ...
      for hycomurl in hycomurls:
         ...
         while not success:
            try:
               ...
               print( '        x '+str(NIT)+' records from '+hycomurl, end='' )
               ITs = []
               ATs = []
         ...
         for hycomdate,itime in z:
            ...
            # /!\ the following print statement requires __future__
            if itime == 0: print( ' from: '+str(d), end='' )
...
Download HyCOM data into a SELAFIN file
      ...
      usage=' (--help for help)\n---------\n       =>  %(prog)s [options]\n---------',
      epilog=('''\nexamples:\n---------
1: Extraction of 2 months of 2D and 3D HYCOM data for ADCP comparison.
      => convertHYCOM.py --from 2013-08-01 --stop 2013-11-21 --bl 20,-119 --tr 29,-108 -r hycom-2m.slf
2: Extraction of 3 years of 2D HYCOM data for big picture review.
      => convertHYCOM.py --from 2010-11-21 --stop 2013-11-21 --bl 20,-119 --tr 29,-108 -r hycom-3y.slf --2d
3: Extraction of 3 years of 2D HYCOM data for big picture review.
