...
from os import path, remove, walk, chdir, mkdir, listdir, system, sep
...
from parserMASCARET import scan_xcas
...
from mascaret import createMascaretFiles
...
#
def mapdiff(a1,a2,notime=False):
   """
      ...
      @notime (optional) allow comparison of result with different time
   ...
   if a1.values.shape != a2.values.shape:
      ...
      print( "a1 shape: "+repr(a1.values.shape) )
      print( "a2 shape: "+repr(a2.values.shape) )
      return None
...
def checkval(a0,eps):
   ...
   print( " "*8+"~> Validation for time (in reference file):"+str(a0.time[0]) )
   # Loop on variables
   value=False
   for ivar in range(nvar):
      err = max(a0.values[0][ivar])
      print( " "*10+"- Difference for variable "+a0.names[ivar]+": "+str(err) )
      if err >= EPS[ivar]:
         print( " "*12+"Epsilon reached"+str(EPS[ivar]) )
         value=True
   return value
...
class ACTION:
   ...
   def addAction(self,actions,rank=''):
      ...
      found = False
      try: found  = isinstance(self.active["deprefs"], basestring)
      except: found  = isinstance(self.active["deprefs"], str)
      if found:
         deprefs = {}
      ...
      found = False
      try: found  = isinstance(self.active["outrefs"], basestring)
      except: found  = isinstance(self.active["outrefs"], str)
      if found:
         outrefs = {}
   ...
   #for action in xmlRoot.findall("action"):
      if xmlChild.tag == "action":
         ...
         for cfgname in xmlConfig:
            ...
            if "config" in do.active.keys() and not oneFound:
               cfgFound = True
               for c in do.active["config"].split(','): cfgFound = cfgFound and ( c in cfgname )
               if cfgFound:
                  oneFound = True
         ...
         if not oneFound:
            raise Exception([{'name':'runXML','msg':'could not find the specified configuration made up of the following string(s) '+do.active["config"]+' on your system.\n      ... do check action '+do.active['xref']+' within '+path.basename(xmlFile)}])

         # ~~ Step 2. Loop over configurations ~~~~~~~~~~~~~~~~~~~~
         for cfgname in xmlConfig:
            cfg = xmlConfig[cfgname]['cfg']
            if "config" in do.active.keys():
               if do.active["config"] not in cfgname: continue
            ...
            if do.active["code"] == 'mascaret':
               ...
               if "run" in doable.split(';'):
                  mascfile=[]
                  mascfile = scan_xcas(path.join(do.active['path'],do.active['target']))
                  for mf in mascfile:
                      copy(path.join(do.active['path'],mf),path.join(do.active['safe'],mf))
                  copy(path.join(do.active['path'],do.active['target']),path.join(do.active['safe'],do.active['target']))
                  chdir(do.active['safe'])
                  createMascaretFiles(cfg, do.active['target'])
                  mascaretexe = cfg['root'] + sep + 'builds'+ sep + cfgname + sep + 'bin' + \
                  sep + 'mascaret' + cfg['SYSTEM']['sfx_exe'] + ' FichierCas.txt'
                  try:
                      updated = system(mascaretexe)
                  except Exception as e:
                     xcpt.append(filterMessage({'name':'runmascaret','msg':'   +> run'},e,bypass))
