...
def subsetVariablesSLF(vars,ALLVARS):
   ...
   if len(ids) < len(v):
      print( "... Could not find "+v+" in "+repr(ALLVARS) )
   ...
   def isMerge(self):
      ...
      for slf in self.slfs[1:]:
         ...
         if len(slf.tags['times']) != len(self.slfs[0].tags['times']): same = False
            ...
            for t1,t2 in zip(slf.tags['times'],self.slfs[0].tags['times']): same = same and ( accuracy > (t1-t2) )
      return same
   ...
   def getSERIES(self,nodes,varsIndexes=[]):
      ...
      if len(self.slfs) > 0:
         ...
         for islf,slf in zip(range(len(self.slfs)),self.slfs):
            ...
            print( '         ~> '+str(len(subLnodes))+' nodes from partition '+str(islf) )
