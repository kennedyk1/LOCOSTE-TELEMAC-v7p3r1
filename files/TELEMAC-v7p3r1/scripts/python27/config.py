...
def parseConfigFile(file,name,bypass=False):
   ...
   for cfgname in configDict:
      ...
      for k in key_records:
         key = k.strip('[]')
         for cfgkey in configDict[cfgname]:
            if key in configDict[cfgname]: configDict[cfgname][cfgkey] = configDict[cfgname][cfgkey].replace(k,configDict[cfgname][key])
      # ~~> defaulting [key] to the environment, if so defined
      for cfgkey in configDict[cfgname]:
         for k in re.findall(key_sqbrack,configDict[cfgname][cfgkey]):
            key = k.strip('[]')
            if key in environ: configDict[cfgname][cfgkey] = configDict[cfgname][cfgkey].replace(k,environ[key])
            else:
...
def parseConfig_ValidateTELEMAC(cfg):
   ...
   # Exception for mascaret as it is not in cfgTELEMAC['MODULES']
   ...
   for moddir in listdir(sources) :
      ...
      for fle in filenames:
         ...
         if f[len(f)-1].lower() in ['.cmdf']:   # /!\ you found at least one cmdf file
            if rescan and ("mascaret" not in f):
...
def getPARTEL(cfgDict):
   ...
   partel = {}
   if 'par_path' in cfgDict: partel.update({'PATH':cfgDict['par_path']})
   if 'par_cmdexec' in cfgDict: partel.update({'EXEC':cfgDict['par_cmdexec']})
   return partel
...
def getMPI(cfgDict):
   # ~~ Loads Compiler Commands ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   mpi = {}
   if 'mpi_cmdexec' in cfgDict:
      mpi.update({'EXEC':cfgDict['mpi_cmdexec']})
      if 'mpi_infile' in cfgDict: mpi.update({'INFILE':cfgDict['mpi_infile']})
      mpi.update({'HOSTS':gethostname().split('.')[0]}) # /!\ defaulting on  the local hostname
      if 'mpi_hosts' in cfgDict:
...
def getHPC(cfgDict):
   # ~~ Loads Compiler Commands ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   hpc = {}
   if 'hpc_cmdexec' in cfgDict:
      hpc.update({'EXCODE':cfgDict['hpc_cmdexec']})
      if 'hpc_stdin' in cfgDict: hpc.update({'STDIN':['HPC_STDIN',cfgDict['hpc_stdin'].replace(r'\n','\n')]})
   if 'hpc_runcode' in cfgDict:
      hpc.update({'PYCODE':cfgDict['hpc_runcode']})
      if 'hpc_stdin' in cfgDict: hpc.update({'STDIN':['HPC_STDIN',cfgDict['hpc_stdin'].replace(r'\n','\n')]})
   if 'hpc_depend' in cfgDict:
...
def parseUserModules(cfgDict,modules):
   ...
   for mod in userList.split():
      if '-' == mod[0:1]:
         print( '\nCould not find the following module: '+repr(mod[1:]) )
         sys.exit(1)
   # ~~ Deal with all ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
