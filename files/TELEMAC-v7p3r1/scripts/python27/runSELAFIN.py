...
class scanSELAFIN(PARAFINS,chopSELAFIN): # /!\ does not support PARAFINS yet -- because of the print order of print Core
   ...
   def printHeader(self):
      ...
      if self.slf.IPARAM[9] == 1:    # /!\ needs proper formatting
         print( "DATE / TIME  : "+ \
            str(self.slf.DATETIME[2])+"-"+str(self.slf.DATETIME[1])+"-"+str(self.slf.DATETIME[0])+" "+ \
            str(self.slf.DATETIME[3])+":"+str(self.slf.DATETIME[4])+":"+str(self.slf.DATETIME[5]) )
      ...
      print( "NUMBERs      :" )
      print( "   - NPLAN* = "+str(self.slf.IPARAM[6])+"\n   - NPTFR* = "+str(self.slf.IPARAM[7])+"\n   - IFACE* = "+str(self.slf.IPARAM[8]) )
      print( "   - NELEM3 = "+str(self.slf.NELEM3)+"\n   - NPOIN3 = "+str(self.slf.NPOIN3)+"\n   - NDP3   = "+str(self.slf.NDP3)+"\n   - NPLAN  = "+str(self.slf.NPLAN) )
      if self.slf.NPLAN > 1: print( "   - NELEM2 = "+str(self.slf.NELEM2)+"\n   - NPOIN2 = "+str(self.slf.NPOIN2)+"\n   - NDP2   = "+str(self.slf.NDP2) )
      if self.slf.NPOIN2 > 0: print( "MESH         : / min: [ "+str(np.min(self.slf.MESHX))+";"+str(np.min(self.slf.MESHY))+"]  / max: [ "+str(np.max(self.slf.MESHX))+";"+str(np.max(self.slf.MESHY))+"]" )
      print( "ARRAYs       :" )
      print( "   - IKLE  : / min: [ "+str(np.min(self.slf.IKLE3)+1)+"]  / max: [ "+str(np.max(self.slf.IKLE3)+1)+"] "+repr(self.slf.IKLE3+1) )
      print( "   - IPOBO : / min: [ "+str(np.min(self.slf.IPOB3))+"]  / max: [ "+str(np.max(self.slf.IPOB3))+"] "+repr(self.slf.IPOB3) )
   ...
   def printCore(self):
      for v in range(self.slf.NBV1):
         ...
         for t in range(len(self.slf.tags['times'])):
            ...
            if self.slf.NPOIN2 > 0: print( "    / TIME: "+str(self.slf.tags['times'][t])+"/ min:"+str(np.min(VARSOR[0]))+"/ max:"+str(np.max(VARSOR[0])) )
   ...
   def printTimeSummary(self):
      print( "NUMBER OF TIMES : "+str(len(self.slf.tags['times'])) )
      print( "FIRST TIME STEP : "+str(self.slf.tags['times'][0]) )
      print( "LAST TIME STEP  : "+str(self.slf.tags['times'][len(self.slf.tags['times'])-1]) )
      if len(self.slf.tags['times']) > 1: print( "TIME STEP       : "+ \
         str( ( self.slf.tags['times'][len(self.slf.tags['times'])-1] - self.slf.tags['times'][0] )/( len(self.slf.tags['times'])-1 ) ) )
...
class transfSELAFIN(PARAFINS,alterSELAFIN):
   ...
   def putContent(self,fileName):
      ...
      # ~~> Time stepping
      print( '\n      > Going through time ('+str(len(self.history))+'time steps) :' )
...
class scanSPECTRAL(scanSELAFIN):
   ...
   def printHeader(self):
      ...
      print( "FREQUENCIES "+str(nf)+" / min: [ "+str(self.slf.MESHY[0])+"]  / max: [ "+str(self.slf.MESHY[self.slf.NPOIN3-nd])+"]" )
      print( ' '.join([ repr(i) for i in self.slf.MESHY[0::nd] ]) )
      print( "DIRECTIONS  "+str(nd)+" / angle: "+str(360./nd) )
   ...
   def printCore(self,na):
      for v in range(self.slf.NBV1):
         ...
         for t in range(len(self.slf.tags['times'])):
            VARSOR = self.slf.getVariablesAt( t,[self.slf.VARINDEX[v]] )
            print( "    / TIME: "+str(self.slf.tags['times'][t]) )
...
Possible actions:\n
   ...
   subdivide   will subdivide a mesh by one iteration (splitting all triangles in four others)
         '''),
         usage=' (--help for help)\n---------\n      =>  %(prog)s action [options] filename(s)\n---------',
         epilog=(''))
      parser.add_argument( "code", default=None, \
         choices=['scan','spec','chop','alter','merge','diff','calcs','crunch','transf','sample','subdivide','tessalate'] )
   else:
      parser = ArgumentParser(\
         formatter_class=RawDescriptionHelpFormatter,
         description=('''\n
One of the tools to handle SELAFIN files and TELEMAC binary related in python
         '''),
         usage=' (--help for help)\n---------\n      =>  %(prog)s [options] filename(s)\n---------',
         epilog=(''))
      parser.set_defaults(code=action)
   ...
   parser.add_argument( "args", metavar='SELAFIN file(s)', nargs="+" )

   options = parser.parse_args()

# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
# ~~~~ Reads code name ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   codeName = options.code

# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
# ~~~~ Case of SCAN ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   if codeName == 'scan':

      slfFiles = options.args
...
# ~~~~ Case of MERGE ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      if not options.parallel:
         if len(options.args) < 3:
            ...
            parser.print_help()
            sys.exit(1)
         slfFiles = options.args[0:len(options.args)-1]
...
# ~~~~ Case of SAMPLE ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      if not options.parallel:
         ...
         outFile = options.args[1]
         nodList = []
         for nod in options.args[2].split(" "): nodList.append(int(nod))
      else:
