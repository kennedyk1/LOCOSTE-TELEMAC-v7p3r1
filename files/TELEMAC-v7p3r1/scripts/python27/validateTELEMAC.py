...
a certain rank, an integer defined as a multiplication (combination) of prime
numbers as follows:
   - 0: (default value), means that that everything goes.
   - 2: rank 2 or remain integer when divided by 2
   - 3: rank 4 or remain integer when divided by 3
   - 5: rank 5 or remain integer when divided by 5
   - ...
which means that, for instance:
   - 6: can be divided by 2 and 3, therefore will include all test cases of rank 2 or 3.
   -210: will include test cases of rank 2, 3, 5 and 7 (2x3x5x7=210)
      '''),
         usage=' (--help for help)\n---------\n      =>  %(prog)s module [options] xmlfile(s)\n---------',
         epilog=('''\nexamples:\n---------
1:    => validateTELEMAC.py xmlfile.xml
2:    => validateTELEMAC.py -k 13
---------'''))
   parser.add_argument(\
...
# Combine with all filters above, "rank" now controls everything and Jenkins can control "rank"
   parser.add_argument(\
      ...
      help="specify the (combination of) rank(s) to be validated (dfualt is 0, i.e. all test cases)" )
   parser.add_argument(\
   ...
   parser.add_argument( "args", metavar='xml file(s)', nargs='*' )
