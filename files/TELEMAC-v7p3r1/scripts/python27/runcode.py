...
def processExecutable(cas,pbin,plib,system,dico,frgb,trace,bypass):
      ...
      if path.exists(oriFile):
         if path.isfile(oriFile): oriFort.append( oriFile )
            ...
            for f in listdir(oriFile):
               if path.isfile(path.join(oriFile,f)): oriFort.append( path.join(oriFile,f) )
   ...
   # ~~> ... and compiling fortran file(s)
      ...
      for f90 in user_files:
         ...
         print( path.splitext(f90)[0]+system['sfx_obj'], end='' )
...
---------'''))
      parser.add_argument( "module", default=None, \
         choices=['telemac2d','telemac3d','artemis','tomawac','stbtel','postel3d','sisyphe','partel','estel3d'] )
   else:
      parser = ArgumentParser(\
         formatter_class=RawDescriptionHelpFormatter,
         description=('''\n
