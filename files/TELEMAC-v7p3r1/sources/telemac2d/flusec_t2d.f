...
!       CASE WHERE NO SEGEMENT WAS ADDED
          IF(FLUXLINEDATA_FLUSECT2D(ISEC)%NOFSECTIONS.LE.2)THEN
            IF(LNG.EQ.1)THEN
              WRITE(LU,*)'FLUSEC_T2D: ATTENTION, LA SECTION (FLUXLINE)'
              WRITE(LU,*)'          ',ISEC,' CONTIENT PEU DE SEGMENTS.'
              WRITE(LU,*)'           CAUSE POSSIBLE: BOX QUI CONTIENT'
              WRITE(LU,*)'           LE FLUXLINE EST TROP PETIT'
            ELSEIF(LNG.EQ.2)THEN
...
!
      IF(DOPLOT) THEN
        WRITE(LU,*)''
        IF(LNG.EQ.1) THEN
          WRITE(LU,*) '  SECTIONS DE CONTROLE (OPTION 2):'
        ELSEIF(LNG.EQ.2) THEN
