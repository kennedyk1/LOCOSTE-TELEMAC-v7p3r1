...
!
      ...
      USE DECLARATIONS_TELEMAC2D, ONLY:MSK,MASKEL,T6,DTINI,LEOPRD,
     &                                 COMPLEO,PTINIG
!
...
!
     ...
     &             CFLWTD,ICIN,DTVARI,LISTIN,LEO,NORDRE)
!
...
!
      IF(NORDRE.EQ.2)THEN
        THEEND =.FALSE.
        LEO    =.FALSE.
        IF((TMAX-AT).LE.DTN)THEEND=.TRUE. !LAST TIME STEP
!       ADAPT DT TO TAKE INTO ACCOUNT GRAPHIC OUTPUT
        IS=CEILING(AT/GPRDTIME)
        RESTE=IS*GPRDTIME-AT
!
        IF(RESTE.GT.1.E-16.AND.LT.GT.PTINIG)THEN
          IF(RESTE.LE.DTN.OR.THEEND)THEN  ! HERE THERE IS GRAPHICAL OUTPUT
             LEO = .TRUE.
             COMPLEO=COMPLEO+1
           ENDIF
           DTN=MIN(RESTE,DTN)
