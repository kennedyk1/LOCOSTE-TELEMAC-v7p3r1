...
!
              IF(S1.LT.(RD1+HAUT1).AND.S1.LT.(RD2+HAUT2)) THEN
!             FREE SURFACE FLOW WHICH FOLLOWS A WEIR LAW
                IF(S2.GT.(TWOTHIRDS*(S1-RD))+RD) THEN
                  Q = LARG*SQRT(2.D0*GRAV*(S1-S2)/(CE1+L+CS2))*(S2-RD2)
                  SECBUS(N) = (S2-RD2) * LARG
                ELSE
                  Q = LARG*SQRT(2.D0*GRAV)*SQRT((S1-RD1)**3)*0.385D0
                  SECBUS(N) = (S1-RD1) * LARG
                ENDIF
              ELSE
!               PRESSURE FLOW --> ORIFICE LAW
                SECBUS(N) = LARG * HAUT1
                IF(S1.GE.(RD1+HAUT1).AND.S2.LT.(RD2+HAUT2)) THEN
                  Q = SECBUS(N)*SQRT(2.D0*GRAV*(S1-(RD2+HAUT2))
     &                              /(1.D0+CE1+L))
                ELSE
...
!
            IF(S2.GT.RD1.AND.S2.GT.RD2) THEN
              ...
              IF(S2.LT.(RD1+HAUT1).AND.S2.LT.(RD2+HAUT1)) THEN
!               FREE SURFACE FLOW WHICH FOLLOWS A WEIR LAW
                IF(S1.GT.(TWOTHIRDS*(S2-RD1)+RD1)) THEN
                  Q = -LARG*SQRT(2.D0*GRAV*(S2-S1)/(CE2+L+CS1))*(S1-RD1)
                  SECBUS(N) = (S1-RD1) * LARG
                ELSE
                  Q = -LARG*SQRT(2.D0*GRAV)*SQRT((S2-RD2)**3)*0.385D0
                  SECBUS(N) = (S2-RD2) * LARG
                ENDIF
              ELSE
!               PRESSURE FLOW --> ORIFICE LAW
                SECBUS(N) = LARG * HAUT1
                IF(S2.GE.(RD2+HAUT2).AND.S1.LT.(RD1+HAUT1)) THEN
...
!
!             FREE SURFACE FLOW
              IF((S1-RD1).LT.1.5D0*HAUT1.AND.S2.LE.(RD2+HAUT2)) THEN
!
...
!
!             PRESSURE FLOW --> ORIFICE LAW
              ELSEIF((S1-RD1).GE.1.5D0*HAUT1.AND.
     &           S2.LE.(RD2+HAUT2)) THEN
!               FLOW TYPE 6
...
!           FLOW TYPE 4 SUBMERGED OUTLET
              ELSEIF(S1.GT.(RD1+HAUT1).AND.S2.GT.(RD2+HAUT2)) THEN
                FTYP=4
                HAST = HAUT1
                RAYON = HAST*LARG/(2.D0*HAST+2.D0*LARG)
                L = 2.D0*GRAV*LONG*FRIC**2/RAYON**FOURTHIRDS
...
!             FREE SURFACE FLOW
              IF((S2-RD2).LT.1.5D0*HAUT2.AND.S1.LE.(RD1+HAUT1)) THEN
!               SUBMERGED WEIR - FLOW TYPE 3
...
!               UNSUBMERGED WEIR - FLOW TYPE 2
     ...
     &                               /(CE2+VALVE+L+CS1+TRASH))
                ENDIF
!             PRESSURE FLOW --> ORIFICE LAW
              ELSEIF((S2-RD2).GE.1.5D0*HAUT2.AND.S1.LE.(RD1+HAUT1)) THEN
!               FLOW TYPE 6
...
!                 FLOW TYPE 5
     ...
     &                                  /(CORR5*CE2+CORRV5*VALVE+TRASH))
                ENDIF
!             FLOW TYPE 4 SUBMERGED OUTLET
              ELSEIF(S2.GT.(RD2+HAUT2).AND.S1.GT.(RD1+HAUT1)) THEN
                FTYP=-4
                HAST = HAUT2
                RAYON = HAST*LARG/(2.D0*HAST+2.D0*LARG)
                L = 2.D0*GRAV*LONG*FRIC**2/RAYON**FOURTHIRDS
...
!       WRONG CHOICE OF OPTBUSE VALUE
        ELSE
          IF(LNG.EQ.1) THEN
            WRITE(LU,*) 'MAUVAISE VALEUR DE LA VARIABLE OPTBUSE'
          ENDIF
          IF(LNG.EQ.2) THEN
