...
!     ERROR IN THE DATA
        IF(LNG.EQ.1) WRITE(LU,102) K, NBOR(K)
        IF(LNG.EQ.2) WRITE(LU,103) K, NBOR(K)
        CALL PLANTE(1)
        STOP
      ELSEIF(L1.NE.KLOG.AND.L2.EQ.KLOG.AND.L3.NE.KLOG) THEN
!     ERROR IN THE DATA
        IF(LNG.EQ.1) WRITE(LU,104) K, NBOR(K)
        IF(LNG.EQ.2) WRITE(LU,105) K, NBOR(K)
        CALL PLANTE(1)
        STOP
      ENDIF
!
