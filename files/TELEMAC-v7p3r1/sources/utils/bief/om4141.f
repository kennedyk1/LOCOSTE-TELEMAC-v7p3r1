...
!
      IF(STOX.EQ.2.AND.OP(3:8).NE.'N     ') THEN
        IF(LNG.EQ.1) THEN
          WRITE(LU,*) 'OM4141 (BIEF) : STOCKAGE EXTRADIAGONAL 2'
          WRITE(LU,*) '                NON PROGRAMME'
          WRITE(LU,*) '                AVEC OPERATION ',OP
        ENDIF
        IF(LNG.EQ.2) THEN
...
!           EVEN IF NELMAX MAY BE LARGER THAN NELEM
            CALL OV( 'X=Y     ',XM,XN,Z,C,15*NELMAX)
          ELSE
            DO I=1,15
...
!           SLICING THE COPY WOULD MAKE TOO MANY LOOPS, THE WHOLE COPIED
!           EVEN IF NELMAX MAY BE LARGER THAN NELEM
            CALL OV( 'X=Y     ' ,XM,XN,Z,C,30*NELMAX)
          ELSE
            DO I=1,30
              CALL OV( 'X=Y     ' ,XM(1,I),XN(1,I),Z,C,NELEM)
            ENDDO
...
!
        IF(TYPEXM(1:1).EQ.'S') THEN
          DO I=1,15
            CALL OV( 'X=Y     ',XM(1,I+15),XM(1,I),Z,C,NELEM)
          ENDDO
        ELSEIF(TYPEXM(1:1).NE.'0') THEN
