...
!-----------------------------------------------------------------------
     ...
     &            TRIM(MYDICO(IKEY)%KNOM(FR))//'</translation>'
        ...
        DO IKEY = 1,NKEY
          ! If we have a keyword with a choix made of string and no = in the
          ! choix
          IF (MYDICO(IKEY)%KTYPE.EQ.4 .AND.
     &        MYDICO(IKEY)%CHOIX(FR)(1:1).NE.' '.AND.
     &        INDEX(MYDICO(IKEY)%CHOIX(FR),"=").EQ.0) THEN
     ...
     &                 '</translation2>'
        ...
        !
        FILENAME = REPEAT(' ',144)
        FILENAME = TRIM(PATH)//'dicoCasEnToCata.py'
     ...
     &               '",'
        ...
        !
        FILENAME = REPEAT(' ',144)
        FILENAME = TRIM(PATH)//'dicoCasFrToCata.py'
