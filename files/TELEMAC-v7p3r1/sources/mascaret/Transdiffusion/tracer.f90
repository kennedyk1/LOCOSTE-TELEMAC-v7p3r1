...
!******************************************************************************
   ...
   if( IPASS == PHASE_INITIALISATION ) then
	  ...
	  !****************************************************************
	  allocate(NB_CHILD(nb_noeud),STAT=retour)
      if( retour /= 0 ) then
         Erreur%Numero = 5
         Erreur%ft   = err_5
         Erreur%ft_c = err_5c
         call TRAITER_ERREUR  (Erreur, 'NB_CHILD')
         !stop
		 return
      end if
	  NB_CHILD(:) = 0
	  allocate(NB_PARENT(nb_noeud),STAT=retour)
      if( retour /= 0 ) then
         Erreur%Numero = 5
         Erreur%ft   = err_5
         Erreur%ft_c = err_5c
         call TRAITER_ERREUR  (Erreur, 'NB_PARENT')
         !stop
		 return
      end if
	  NB_PARENT(:) = 0
	  allocate(CHILD(nb_noeud,nb_noeud),STAT=retour)
      if( retour /= 0 ) then
         Erreur%Numero = 5
         Erreur%ft   = err_5
         Erreur%ft_c = err_5c
         call TRAITER_ERREUR  (Erreur, 'CHILD')
         !stop
		 return
      end if
	  allocate(PARENT(nb_noeud,nb_noeud),STAT=retour)
      if( retour /= 0 ) then
         Erreur%Numero = 5
         Erreur%ft   = err_5
         Erreur%ft_c = err_5c
         call TRAITER_ERREUR  (Erreur, 'PARENT')
         !stop
		 return
      end if
	  !***********************************************************
	     ...
	     do IP = 1,Connect%NbBiefConfluence(inoeud)
            ...
            if( Connect%FinBief(num_bief) == num_sect ) then
		       ...
		       do inoeud0 = 1,nb_noeud
			      ...
			      if(arret.eqv..true.) then
				     NB_PARENT(inoeud) = NB_PARENT(inoeud) + 1
					 PARENT(inoeud,NB_PARENT(inoeud)) = inoeud0
				  endif
		       enddo
            endif
            ! Branche aval
			if( Connect%OrigineBief(num_bief) == num_sect ) then
		       ...
		       do inoeud0 = 1,nb_noeud
			      ...
			      if(arret.eqv..true.) then
				     NB_CHILD(inoeud) = NB_CHILD(inoeud) + 1
					 CHILD(inoeud,NB_CHILD(inoeud)) = inoeud0
				  endif
		       enddo
            endif
         enddo
         ...
         if( nb_noeud >= 1 ) then
            ...
            do while(nok.ne.0)
               do INOEUD = 1 , nb_noeud
                  ...
                  arret = .false.
                  do IP = 1,NB_PARENT(inoeud)
                     if(nodeOK(PARENT(inoeud,IP)).eqv..false.) then
                     arret = .true.
	                 exit
                  endif
               enddo
