...
!      end if
   ...
   if( PhaseSimulation == PHASE_CALCUL ) then
      ...
      ! ---------------------
!      write(*,*) 'Avant CALCUL (RESOLUTION DU SYSTEME)'
!      nomFichier = 'etatAvantCalculRezo.xml'
!      call EXPORT_XML(num_erreur, 2, nomFichier, .true., .false.)
!      nomFichier = 'ModeleAvantCalculRezo.xml'
!      call EXPORT_XML(num_erreur, 2, nomFichier, .true., .true.)
      call CALCUL                                     ( &
            Z, Q                                      , &
            Matrice                                   , &
...
!      nomFichier = 'etatApresCalculRezo.xml'
!      call EXPORT_XML(num_erreur, 2, nomFichier, .true., .false.)
!      nomFichier = 'ModeleApresCalculRezo.xml'
!      call EXPORT_XML(num_erreur, 2, nomFichier, .true., .true.)

      if( Erreur%Numero /= 0 ) then
         write(*,*) 'Erreur dans CALCUL (RESOLUTION DU SYSTEME)'
         return
      end if
   ...
   ! De allocations de tableaux locaux
   !----------------------------------
