...
subroutine  INIT_LIGNE_MASCARET(Erreur, Identifiant, Q, Z, Taille)
   ...
   Erreur = TEST_INIT_AND_ID(Identifiant, 'INIT_LIGNE_MASCARET')
   if (Erreur > 0 ) then
      RETURN
   end if
...
!---------------------
   ...
   if (taille /= nb_sect) then
      Erreur = 2
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - The numbers of nodes are different'
      return
   endif

   retour = 0
   if(.not.associated(Etat%Z)) allocate (Etat%Z(nb_sect), STAT = retour)
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.Z'
      return
   endif
   do i = 1, nb_sect
       Etat%Z(i) = Z(i)
   ...
   if(.not.associated(Etat%Q)) allocate (Etat%Q(nb_sect), STAT = retour)
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.Q'
      return
   endif

   Etat%Q(1:nb_sect) = Q(1:nb_sect)
...
!======================================================
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.DPDZ1'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.DPDZ2'
      return
   endif
...
! Allocation des variables conmunes a LIDO et MASCARET
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.V1'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.Qspilled'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.Qinflow'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.Q2'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.V2'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.Y'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.S1'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.S2'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.BETA'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.Froude'
      return
   endif
   ...
   if( retour /= 0 ) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.FLUX'
      return
   end if
   Etat%Flux(:,:) = W0
   ...
   if( retour /= 0 ) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.Flow'
      return
   end if
   Etat%DebitFlux (:) = W0
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.P1'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.P2'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.B1'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.B2'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.BS'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.RH1'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.RH2'
      return
   endif
...
! Allocation des variables specifiques a MASCARET
   if (Model%Noyau == NOYAU_MASCARET) then
      ...
      if (retour /= 0) then
         Erreur = retour
         ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.XFRON'
         return
      endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.UNODE'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.CNODE'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.YNODE'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.JGNODE'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.JDNODE'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.IFIGE'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.W'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.AIRS'
      return
   endif
...
! Allocation des variables specifiques a Casier
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.Link'
      return
   endif
   ...
   if (retour /= 0) then
      Erreur = retour
      ptrMsgsErreurs(Identifiant) = 'INIT_LIGNE_MASCARET - Unable to allocate State.StoArea'
      return
   endif
