...
subroutine LEC_LIGNE_INTERFACE( &
                      Z , & ! Cote initiale
                      Q , & ! debit initial
...
!                 MOTINT , & ! Tableaux provenant de DAMOCLES
!                 MOTREA , & ! Tableaux provenant de DAMOCLES
!                 ADRESS , & ! Tableaux provenant de DAMOCLES
                 Erreur ) ! Erreur
...
!   integer       , dimension(:)                 :: MOTINT
!   real(DOUBLE)  , dimension(:)                 :: MOTREA
!   integer       , dimension(:,:)               :: ADRESS
...
!   if( TypeEntreeLigne == SAISIE_PAR_FICHIER ) then
      if( FormatLigne == FORMAT_STO_OPTHYCA ) then
         ...
         if(associated(x_ini_abs)) deallocate(x_ini_abs)
         allocate( x_ini_abs(size(x_ini)) , STAT = retour )
         if( retour /= 0 ) then
            Erreur%Numero = 5
            Erreur%ft     = err_5
            Erreur%ft_c   = err_5c
         ...
         if( (.not.Prof_Abs) .and. ( TypeMaillage /= TYPE_MAILLAGE_PRECEDENT ) ) then
            ...
            do j = 1 , size(AbsRelDebBief)
               ...
               do while (x_ini(i) /= AbsRelFinBief(j))
                  i = i + 1
               end do
         ...
         if( (.not.Prof_Abs) .and. ( TypeMaillage /= TYPE_MAILLAGE_PRECEDENT ) ) then
            ...
            do j = 1 , size(AbsRelDebBief)
               ...
               do i = ideb_bief, ifin_bief
                  ...
                  if( Erreur%Numero /= 0 ) then
                     !write(*,*) 'ABS_ABS_S Erreur%Message:',Erreur%Message
                     return
                  end if
   ...
   !-----------------------------
   ! Sinon lecture sur le clavier
   !-----------------------------
!   else if( TypeEntreeLigne == SAISIE_PAR_CLAVIER ) then

!      nb_sect = MOTINT(ADRESS(1,14))
...
!      allocate( z_ini(nb_sect) , STAT = retour )
!      if( retour /= 0 ) then
!         Erreur%Numero = 5
!         Erreur%ft     = err_5
!         Erreur%ft_c   = err_5c
...
!      allocate( q_ini(nb_sect) , STAT = retour )
!      if( retour /= 0 ) then
!         Erreur%Numero = 5
!         Erreur%ft     = err_5
!         Erreur%ft_c   = err_5c
...
!         section%Branche     = MOTINT(ADRESS(1,15)+i-1)
!         section%AbscisseRel = MOTREA(ADRESS(2,8)+i-1)

         ! Controle des points de rebroussements
!         if( section%Branche == branche_prec .and. section%AbscisseRel <= x_prec ) then
!            Erreur%Numero = 377
!            Erreur%ft     = err_377
!            Erreur%ft_c   = err_377c
...
!         z_ini(i) = MOTREA(ADRESS(2,9)+i-1)
!         q_ini(i) = MOTREA(ADRESS(2,10)+i-1)

!         branche_prec = section%Branche
!         x_prec       = section%AbscisseRel

!      end do

!      presence_coeff_frott = .false.
...
!   endif
   ...
   if( TypeMaillage == TYPE_MAILLAGE_PRECEDENT ) then
      ...
      allocate( X(nb_sect) , STAT = retour )
      if( retour /= 0 ) then
         Erreur%Numero = 5
         Erreur%ft     = err_5
         Erreur%ft_c   = err_5c
   ...
   allocate( Z(nb_sect) , STAT = retour )
   if( retour /= 0 ) then
      Erreur%Numero = 5
      Erreur%ft     = err_5
      Erreur%ft_c   = err_5c
   ...
   allocate( Q(nb_sect) , STAT = retour )
   if( retour /= 0 ) then
      Erreur%Numero = 5
      Erreur%ft     = err_5
      Erreur%ft_c   = err_5c
   ...
   if( TypeMaillage == TYPE_MAILLAGE_PRECEDENT .and. presence_coeff_frott ) then
      ...
      if(associated(CF1)) deallocate(CF1)
      allocate( CF1(nb_sect) , STAT = retour )
      if( retour /= 0 ) then
         Erreur%Numero = 5
         Erreur%ft     = err_5
         Erreur%ft_c   = err_5c
      ...
      if(associated(CF2)) deallocate(CF2)
      allocate( CF2(nb_sect) , STAT = retour )
      if( retour /= 0 ) then
         Erreur%Numero = 5
         Erreur%ft     = err_5
         Erreur%ft_c   = err_5c
...
end subroutine LEC_LIGNE_INTERFACE
