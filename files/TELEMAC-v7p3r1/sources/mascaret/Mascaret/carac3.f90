...
! SGEO, AIGEO et DYGEO font partie d'une structure de donnees
   ...
   real(DOUBLE)                      :: UCOUR,DXPIED,XPD,UNG,UND,CNG,CND,SNG,SND
   ...
   !Erreur%arbredappel = trim(!Erreur%arbredappel)//'=>CARAC3'

   IMP1 = NSECD + 1

   ! CALCUL DES VALEURS CONVECTEES ET DES VITESSES DE CONVECTION
   do NOEUD = NSECG , NSECD
      FN(NOEUD)  = UNODE(NOEUD) + TCAR * AKNODE(NOEUD)
      VIT(NOEUD) = UNODE(NOEUD) + TCAR * CNODE(NOEUD)
   end do
   ...
   do 2000 I = NSECG , NSECD
      ...
      if( NG == NSECG - 1 ) then
         !          SORTIE A GAUCHE
         if( Impression ) then
            write (UniteListing,9010) NSECG
         endif
9010     format ('SSP CARAC3 ; SORTIE A GAUCHE AU NOEUD GLOBAL',I5)            
         UREM  =  UNODE(NSECG)
         CREM  =  CNODE(NSECG)
         SREM  =  SNODE(NSECG)
         AKREM = AKNODE(NSECG)
      ...
      if( NG == NSECD ) then
         !          SORTIE A DROITE
         if( Impression ) then
            write(UniteListing,9020) NSECD
         endif
9020     format ('SSP CARAC3 ; SORTIE A DROITE AU NOEUD GLOBAL',I5)
         UREM  = UNODE(NSECD)
         CREM  = CNODE(NSECD)
         SREM  = SNODE(NSECD)
         AKREM = AKNODE(NSECD)
      ...
      SNG   = SNODE(NG)
      SND   = SNODE(ND)
      UREM  = ( UNG * ( DX - XPD ) + UND * XPD ) / DX
      CREM  = ( CNG * ( DX - XPD ) + CND * XPD ) / DX
      SREM  = ( SNG * ( DX - XPD ) + SND * XPD ) / DX
      ...
      do 60 I = NSECG , NSECD
         ...
         FNP1(NSECG) = FNP1(NSECG) &
                     ...
                     *    ( 2._DOUBLE * SLOPE(NSECG) + SLOPE(NSECG+1) ) * GPES * DT / 2._DOUBLE &
                     -    ( X(NSECG+1) - X(NSECG) ) * ( 2._DOUBLE * FROT(NSECG) + FROT(NSECG+1) ) * GPES * DT / 2._DOUBLE
         do I = NSECG + 1 , NSECD - 1
            FNP1(I) = FNP1(I) &
