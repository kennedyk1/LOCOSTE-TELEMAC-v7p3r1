...
! !  EPS      !  R !  A ! PRECISION DANS LA RESOLUTION PAR             !
...
!*****************************************************************************
   ...
   integer      :: RINV
   integer      :: MAXFEN
   real(DOUBLE) :: YGAUC,YDROI,YECH,EPS
   real(DOUBLE) :: BG,BD,CRG,WR,URG,YRG,ALARG,XBAR
   ...
   !Erreur%arbredappel = trim(!Erreur%arbredappel)//'=>RIEMAN'
   ...
   if( YDROI < EPS2 ) then
      ! RUPTURE SUR FOND SEC
      YDROI = EPS8
      SDROI = YDROI * ALARG
   endif

   if( ITEMP == 1 .and. Impression ) then
      ! PREMIER PASSAGE DANS LA SUBROUTINE SOLEX
      ! ========================================
      write(UniteListing,1000)
      write(UniteListing,1010) XBAR , ALARG
      write(UniteListing,1020) YGAUC , YDROI
   endif
   ...
   call ZBRENT( FC1 , EPS , BG , BD , MAXFEN , impression , Erreur )
   if( Erreur%Numero /= 0 ) then
      return
   endif

   CRG = BD
   WR  = ( CRG / CDROI ) * dsqrt( ( CRG**2+ CDROI**2 ) / 2._DOUBLE )
   URG = WR - ( CDROI / CRG ) * dsqrt( ( CRG**2+ CDROI**2 ) / 2._DOUBLE )
   YRG = CRG**2 / GPES
...
1000  format(/,2X,'===================================',/ &
       ...
       ,2X,'===================================',// &
       ,2X,'RUPTURE DE BARRAGE SANS FROTTEMENT',/)
1010 format(2X,'LES PARAMETRES UTILISES POUR CE CALCUL SONT :',/ &
       3X,'- POSITION DU BARRAGE : XBAR  =',F7.2,/ &
       3X,'- LARGEUR DU CANAL    : ALARG =',F7.2,// &
       2X,'SI CES VALEURS SONT INCORRECTES, ' &
       ,'VEUILLEZ LES MODIFIER DANS LA ROUTINE RIEMMAN'/) 
1020 format (2X,'HAUTEUR D''EAU A L''AMONT DU BARRAGE :',F7.3,/ &
       2X,'HAUTEUR D''EAU A L''AVAL DU BARRAGE  :',F7.3)           
