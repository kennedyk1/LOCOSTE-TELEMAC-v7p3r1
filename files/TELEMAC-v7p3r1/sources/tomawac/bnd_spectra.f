...
!
!##> JR @ ADJOINTWARE: MODULE VARIABLE WITH DEFAULT INITIALISATION FOR USER
!     TYPE COMPONENTS MUST BE SAVED (FOR AD VECTOR MODE)
      SAVE :: TV1, TV2, TIME1, TIME2
!##< JR @ ADJOINTWARE
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
...
!    DIRECTIONS AS IN THE SIMULATION
        ...
        BC_RAISF=0.D0
        DO IPOIN=1,BC_NPOIN
          IF((ABS(BC_X(IPOIN)).LE.EPSILON).AND.(BC_Y(IPOIN).GE.0.0))THEN
            BC_NF=BC_NF+1
            F_TEMP=BC_Y(IPOIN)
