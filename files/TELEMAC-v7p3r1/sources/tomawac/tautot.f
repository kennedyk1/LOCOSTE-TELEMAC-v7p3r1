...
!     """""""""""""""""""""""""""""""""""
      USTO  = SQRT(TAUO)
      X     = TAUW/TAUO
      IF(ABS(1.D0-X).LT.1.D-300) THEN
        IF(LNG.EQ.1) THEN
          WRITE(LU,*) 'DANS LA ROUTINE TAUTOT JE DIVISE PAR 1-X'
          WRITE(LU,*) 'ET X=',X,' VENT TROP FORT ET MODELE =1 '
          WRITE(LU,*) 'ESSAYEZ AVEC MODELE =2 ?'
        ENDIF
        IF(LNG.EQ.2) THEN
          WRITE(LU,*) 'IN ROUTINE TAUTOT I DIVIDE BY 1-X'
          WRITE(LU,*) 'AND X=',X,' WIND TOO STRONG AND MODELE =1 ?'
