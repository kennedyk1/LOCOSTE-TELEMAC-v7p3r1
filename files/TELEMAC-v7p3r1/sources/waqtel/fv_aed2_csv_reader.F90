...
!
!-------------------------------------------------------------------------------
   PUBLIC aed_csv_read_header, aed_csv_read_row, aed_csv_close
...
!###############################################################################
FUNCTION extract_number(sym) RESULT(num)
!-------------------------------------------------------------------------------
!
!-------------------------------------------------------------------------------
!ARGUMENTS
...
!ARGUMENTS
    ...
    CHARACTER(len=32),POINTER :: titles(:)
!
!LOCALS
    TYPE(AED_SYMBOL) :: sym
    INTEGER          :: count, i
...
!ARGUMENTS
   ...
   CHARACTER(len=32),DIMENSION(:),POINTER :: names
...
!ARGUMENTS
   ...
   AED_REAL,DIMENSION(:),ALLOCATABLE,INTENT(inout) :: values
!
!LOCALS
   TYPE(AED_READER),POINTER :: aedr
   INTEGER :: i, j, ncols
...
!BEGIN
   ...
   DO WHILE ( next_symbol(aedr, sym) ) !#
      ...
      i = i + 1
      IF ( i <= ncols ) values(i) = extract_number(sym)
   ENDDO
   IF ( i > 0 .AND. i /= ncols ) &
