...
!                          ************************
!
      (COLUMN, COL, CC, CC_DIAG, FLUX_PEL, FLUX_ATM, FLUX_BEN)
!
!***********************************************************************
! WAQTEL      V7P3
!***********************************************************************
...
!                          *************************
!
      (NCELLS, NCOLS, FLUXAED2, EXTCAED2, TA)
!
!***********************************************************************
! WAQTEL      V7P3
!***********************************************************************
...
!# ONLY FOR STATE_VARS THAT ARE NOT SHEET
              IF ( .NOT. ISNAN(TV%MOBILITY) ) THEN
                ...
                CALL SETTLING(BOT-TOP+1,DTAED2,H_AED2(TOP:BOT),        &
...
!# (THIS INCLUDES WATER COLUMN, SURFACE AND BENTHIC INTERFACES)
        CALL CALCULATE_FLUXES(COLUMN,BOT-TOP+1,FLUX(:,TOP:BOT),        &
                              FLUX_ATM,FLUX_BEN,H_AED2(TOP:BOT))

!# FIND THE PARTICLES IN THIS COLUMN AND UPDATE PARTICLE BGC
        IF(DO_PARTICLE_BGC) THEN
          CALL PARTICLES(COLUMN,BOT-TOP+1,H_AED2(TOP:BOT))
        ENDIF
...
!# CHECK FOR NANS
            IF ( ISNAN(CC(I,LEV)) ) THEN
              ...
              WRITE(LU,*)'H_AED2(LEV) = ', H_AED2(LEV),                &
...
!# CHECK FOR NANS
            IF ( ISNAN(CC(I,BOT)) ) THEN
              ...
              WRITE(LU,*)'H_AED2(BOT) = ',H_AED2(BOT),                 &
...
!  EXPORT OF SOURCE TERMS
        DO LEV = TOP, BOT
          DO I = 1, N_VARS
            FLUXAED2(COL,I,BOT-LEV+1)= FLUX(I,LEV)
          ENDDO !N_VARS
          EXTCAED2(COL,BOT-LEV+1) = EXTCOEFF(LEV)
        ENDDO !LEV
      ENDDO ! COLS
!OMP END DO
