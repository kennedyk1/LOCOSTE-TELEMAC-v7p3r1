...
!                    *************************
     ...
     &   ZPROP,T1,T2_1,T3,T4,T5,T6,T7,T8,T9,T10,T11,T12,DEBUG,UN,VN)
!
!
!***********************************************************************
...
!***********************************************************************
     ...
     &  O2NITRI,DEMBEN,MEXTINC,SECTODAY,WAQTR,RANKTR,IND_T,
...
!
      CALL RAY_EFFECT(ZSD,TN%ADR(IND_PHY)%P,NPOIN3,MEXTINC,I0,IK,KPE,
     &                RAYEFF,ZPROP,T3,T4)
!
...
!       - FLOR/H = WLOR[L]/H
     ...
     &           WLOR*TN%ADR(IND_OL)%P%R(J)/MAX(EPS,ZPROP%R(J)      )
      ENDDO
!
...
!       K2g4(Cs-[O2]) - BEN/H
     ...
     &    K2%R(I)*T9%R(J)*MAX((T2_1%R(I)-TN%ADR(IND_O2)%P%R(J)),0.D0 )
...
!
      DO J = 1,WAQTR
        ITRAC = RANKTR(J)
        CALL OS('X=CX    ',X=TEXP%ADR(ITRAC)%P,C=SECTODAY)
      ENDDO
!
      CALL OS('X=CX    ',X=TIMP%ADR(IND_NH4)%P,C=SECTODAY)
      CALL OS('X=CX    ',X=TIMP%ADR(IND_OL)%P,C=SECTODAY)
!
