...
!##>JR @ ADJOINTWARE: NO DATA STATEMENT FOR TYPES WITH ALLOCATABLE COMPONENTS
!      DATA             ZERO/1.D-7/
...
! Pourquoi pas NOMBLAY?
                DO K=3,MIN(NOMBLAY-1,NLAYER%I(J))
                  ES(J,K) = ES(J,K+1)
                ENDDO
!RK last layer must be zeroed, AVAIL: first class = 1, others = 0
...
! Fraction 1 should be 1, if all the others are zero
                ...
                ELAY%R(J) = HEIGH
                ESTRATNEW(J) = 0.D0
!RK a layer was just canceled, the layer thickness of the canceled layer should be set to zero
!   for graphic printout and restart
