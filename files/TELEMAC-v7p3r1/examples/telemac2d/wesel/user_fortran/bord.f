...
!
      IF(NFRLIQ.GE.1) THEN
        ...
        ENDDO
      ENDIF
      ! Modifying UBOR on boundary
      IF(LT.EQ.1) THEN
        DO K=1,NPTFR
          I = NBOR(K)
          IF(NCSIZE.GT.1) I = MESH%KNOLG%I(I)
          IF(I.EQ.442) UBOR(K,1) = 0.3D0
          IF(I.EQ.442) UBOR(K,2) = 0.3D0
          IF(I.EQ.443) UBOR(K,1) = 0.6D0
          IF(I.EQ.443) UBOR(K,2) = 0.6D0
          IF(I.EQ.444) UBOR(K,1) = 1.0D0
          IF(I.EQ.444) UBOR(K,2) = 1.0D0
          IF(I.EQ.445) UBOR(K,1) = 1.0D0
          IF(I.EQ.445) UBOR(K,2) = 1.0D0
          IF(I.EQ.446) UBOR(K,1) = 1.0D0
          IF(I.EQ.446) UBOR(K,2) = 1.0D0
          IF(I.EQ.447) UBOR(K,1) = 1.0D0
          IF(I.EQ.447) UBOR(K,2) = 1.0D0
          IF(I.EQ.448) UBOR(K,1) = 1.0D0
          IF(I.EQ.448) UBOR(K,2) = 1.0D0
          IF(I.EQ.449) UBOR(K,1) = 0.6D0
          IF(I.EQ.449) UBOR(K,2) = 0.6D0
          IF(I.EQ.450) UBOR(K,1) = 0.3D0
          IF(I.EQ.450) UBOR(K,2) = 0.3D0
        ENDDO
      ENDIF
!
!-----------------------------------------------------------------------
