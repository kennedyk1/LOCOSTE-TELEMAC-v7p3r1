...
!
      ...
      INTEGER, INTENT(IN) :: PROVEL(NFRLIQ)
!##> RK @ BAW: INOUT PROPERTY REQUIRED
...
!
      IF(NFRLIQ.GE.1) THEN
        ...
        ENDDO
      ENDIF
      ! Initialise velocity on boundary 2
      IF(LT.EQ.1) THEN
        DO I=1,NPTFR
          K = NBOR(I)
          IF(NCSIZE.GT.1) K = MESH%KNOLG%I(K)
          IF(K.EQ.114) UBOR(I,1) = 0.2D0
          IF(K.EQ.114) UBOR(I,2) = 0.2D0
          IF(K.EQ.258) UBOR(I,1) = 0.6D0
          IF(K.EQ.258) UBOR(I,2) = 0.6D0
          IF(K.EQ.274) UBOR(I,1) = 1.0D0
          IF(K.EQ.274) UBOR(I,2) = 1.0D0
          IF(K.EQ.80)  UBOR(I,1) = 1.0D0
          IF(K.EQ.80)  UBOR(I,2) = 1.0D0
          IF(K.EQ.255) UBOR(I,1) = 1.0D0
          IF(K.EQ.255) UBOR(I,2) = 1.0D0
          IF(K.EQ.207) UBOR(I,1) = 1.0D0
          IF(K.EQ.207) UBOR(I,2) = 1.0D0
          IF(K.EQ.130) UBOR(I,1) = 1.0D0
          IF(K.EQ.130) UBOR(I,2) = 1.0D0
          IF(K.EQ.229) UBOR(I,1) = 1.0D0
          IF(K.EQ.229) UBOR(I,2) = 1.0D0
          IF(K.EQ.117) UBOR(I,1) = 1.0D0
          IF(K.EQ.117) UBOR(I,2) = 1.0D0
          IF(K.EQ.176) UBOR(I,1) = 0.6D0
          IF(K.EQ.176) UBOR(I,2) = 0.6D0
          IF(K.EQ.29)  UBOR(I,1) = 0.2D0
          IF(K.EQ.29)  UBOR(I,2) = 0.2D0
        ENDDO
      ENDIF
!
!-----------------------------------------------------------------------
